...
//
// FlushResolveBatch coalesces pending indices into contiguous ranges to reduce
// ResolveQueryData call count.
//
// ROV queries use a separate counter buffer because pixel-shader-interlock
// depth/stencil tests happen in the translated shader rather than fixed
// function depth. QueueQueryResolve + ClearROVCounter are used instead of
// BeginQuery and EndQuery on that path.
class D3D12ZPDQueryPool {
 public:
  ...
  bool has_pending_resolve_batch() const {
    return !resolve_batch_indices_.empty() || !rov_counter_resolve_batch_indices_.empty();
  }
  ...
  ID3D12Resource* rov_counter_buffer() const { return rov_counter_buffer_.Get(); }
  ...
  void ClearROVCounter(DeferredCommandList& deferred_command_list, uint32_t query_index) const;

  void FlushResolveBatch(DeferredCommandList& deferred_command_list, bool submission_open);

  uint64_t GetQueryReadbackValue(uint32_t query_index, bool uses_rov_counter) const;
 ...
 private:
  ...
  // Persistently mapped. Results are readable once the fence signals.
  ...
  // Bumped on each acquire so stale readbacks from a recycled slot are dropped.
