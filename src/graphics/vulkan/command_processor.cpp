  ...
  for (uint32_t i = 0; i < xenos::kMaxColorRenderTargets; ++i) {
    ...
    *reinterpret_cast<int32_t*>(&color_exp_bias_scale) =
    ...
    if (!edram_fragment_shader_interlock) {
      ...
      // written to the higher-precision host format (e.g. R16G16B16A16_SFLOAT
      // for 7e3) - used both as blend sources and read back for HDR tonemapping
      // - match the Xbox 360. Provide the guest format here; an out-of-range
      // value (UINT32_MAX) disables the rounding for the render target.
