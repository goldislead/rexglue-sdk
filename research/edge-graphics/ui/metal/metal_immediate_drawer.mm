...
MetalImmediateDrawer::MetalImmediateDrawer(MetalProvider* provider) : provider_(provider) {
  device_ = provider_->GetDevice();
}

MetalImmediateDrawer::~MetalImmediateDrawer() { Shutdown(); }
...
static MTL::Function* CreateFunction(MTL::Device* device, const void* data, size_t size,
                                     const char* name) {
  ...
  // XeSL MSL uses entry_xe as the entry point
  NS::String* ns_name = NS::String::string("entry_xe", NS::UTF8StringEncoding);
  MTL::Function* fn = lib->newFunction(ns_name);
  lib->release();
  if (!fn) {
    XELOGE("MetalImmediateDrawer: Failed to load function entry_xe for {}", name);
  }
  return fn;
}
...
bool MetalImmediateDrawer::Initialize() {
  ...
  // Load Shaders
  MTL::Function* vs =
      CreateFunction(device_, immediate_vs_metallib, sizeof(immediate_vs_metallib), "immediate_vs");
  if (!vs) return false;

  MTL::Function* ps =
      CreateFunction(device_, immediate_ps_metallib, sizeof(immediate_ps_metallib), "immediate_ps");
  if (!ps) {
    vs->release();
    return false;
  }
  ...
  // Position attribute (float2) -> entry_xe_stageInput(xesl_float2, xe_in_position, 0, POSITION)
  // Vertex shader input:
  // entry_xe_stageInput(xesl_float2, xe_in_position, 0, POSITION)
  // entry_xe_stageInput(xesl_float2, xe_in_texcoord, 1, TEXCOORD)
  // entry_xe_stageInput(xesl_uint, xe_in_color, 2, COLOR)

  // Attribute 0: Position
  vertex_descriptor->attributes()->object(0)->setFormat(MTL::VertexFormatFloat2);
  vertex_descriptor->attributes()->object(0)->setOffset(offsetof(ImmediateVertex, x));
  ...
  vertex_descriptor->attributes()->object(1)->setOffset(offsetof(ImmediateVertex, u));
  ...
  vertex_descriptor->attributes()->object(2)->setOffset(offsetof(ImmediateVertex, color));
...
std::unique_ptr<ImmediateTexture> MetalImmediateDrawer::CreateTextureFromMetal(
    MTL::Texture* texture, MTL::SamplerState* sampler) {
  if (!texture) {
    return nullptr;
  }
  auto wrapped = std::make_unique<MetalImmediateTexture>(static_cast<uint32_t>(texture->width()),
...
void MetalImmediateDrawer::Begin(UIDrawContext& ui_draw_context, float coordinate_space_width,
  ...
  current_render_encoder_ =
      (__bridge MTL::RenderCommandEncoder*)metal_ui_draw_context.render_encoder();
...
void MetalImmediateDrawer::BeginDrawBatch(const ImmediateDrawBatch& batch) {
  ...
  if (!vertex_buffer) {
    XELOGE("Metal immediate drawer failed to create vertex buffer for {} vertices",
           batch.vertex_count);
    return;
  }
...
void MetalImmediateDrawer::Draw(const ImmediateDraw& draw) {
  ...
  // immediate.ps.xesl: sampler at set=0 binding=0 -> MSL texture(0) + sampler(0).
  ...
  if (current_index_buffer_) {
    ...
    [encoder drawPrimitives:primitive_type vertexStart:draw.base_vertex vertexCount:draw.count];
  }
}
