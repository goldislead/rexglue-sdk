...
bool VulkanZPDQueryPool::EnsureInitialized(const ui::vulkan::VulkanDevice* vulkan_device,
  ...
  // Requires VK_EXT_host_query_reset (1.2 core) to reset slots on the CPU.
  if (!vulkan_device_->properties().hostQueryReset || dfn.vkResetQueryPool == nullptr) {
    return false;
  }
  ...
  if (dfn.vkCreateQueryPool(device, &pool_info, nullptr, &query_pool_) != VK_SUCCESS) {
    REXGPU_WARN(
        "VulkanZPDQueryPool: Failed to create the ZPD query pool, "
  ...
  if (dfn.vkCreateBuffer(device, &readback_buffer_info, nullptr, &readback_buffer_) !=
      VK_SUCCESS) {
    REXGPU_WARN(
        "VulkanZPDQueryPool: Failed to create the ZPD query readback buffer, "
        "falling back to fake sample counts.");
    Shutdown();
    return false;
  }
  ...
  dfn.vkGetBufferMemoryRequirements(device, readback_buffer_, &readback_mem_reqs);
  ...
  if (readback_alloc_info.memoryTypeIndex == UINT32_MAX ||
      dfn.vkAllocateMemory(device, &readback_alloc_info, nullptr, &readback_memory_) !=
          VK_SUCCESS) {
    REXGPU_WARN(
        "VulkanZPDQueryPool: Failed to allocate ZPD query readback memory, "
        "falling back to fake sample counts.");
    Shutdown();
    return false;
  }
  ...
  if (dfn.vkBindBufferMemory(device, readback_buffer_, readback_memory_, 0) != VK_SUCCESS) {
    REXGPU_WARN(
        "VulkanZPDQueryPool: Failed to bind ZPD query readback buffer memory, "
        "falling back to fake sample counts.");
    Shutdown();
    return false;
  }
  ...
  if (dfn.vkMapMemory(device, readback_memory_, 0, VK_WHOLE_SIZE, 0, &mapping) != VK_SUCCESS) {
    REXGPU_WARN(
        "VulkanZPDQueryPool: Failed to map ZPD query readback memory, "
        "falling back to fake sample counts.");
    Shutdown();
    return false;
  }
...
void VulkanZPDQueryPool::ReleaseQueryIndex(uint32_t query_index, uint32_t query_generation) {
  ...
  if (!GenerationMatches(query_index, query_generation)) {
    REXGPU_WARN("VulkanZPDQueryPool: stale release index={} gen={}", query_index, query_generation);
    return;
  }
...
void VulkanZPDQueryPool::RecordResolveBatch(VkCommandBuffer command_buffer) {
  ...
  for (const ResolveRange& range : resolve_batch_ranges_) {
    ...
    VkDeviceSize offset = static_cast<VkDeviceSize>(range.start) * sizeof(uint64_t);
    VkDeviceSize size = static_cast<VkDeviceSize>(count) * sizeof(uint64_t);

    dfn.vkCmdCopyQueryPoolResults(command_buffer, query_pool_, range.start, count,
  ...
  dfn.vkCmdPipelineBarrier(command_buffer, VK_PIPELINE_STAGE_TRANSFER_BIT,
                           VK_PIPELINE_STAGE_HOST_BIT, 0, 0, nullptr, 1, &readback_barrier, 0,
                           nullptr);
}

void VulkanZPDQueryPool::InvalidateReadback() {
  if (readback_is_coherent_ || !vulkan_device_ || readback_memory_ == VK_NULL_HANDLE ||
      !readback_mapping_) {
    return;
  }
