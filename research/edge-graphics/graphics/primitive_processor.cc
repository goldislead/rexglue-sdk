      ...
      switch (guest_primitive_type) {
        case xenos::PrimitiveType::kTriangleFan:
          ...
          if (builtin_ib_offset_triangle_fans_to_lists_ != SIZE_MAX) {
            ...
            // No pre-baked IB: backend didn't opt into global fan-to-list
            // conversion at init. Reached via the tessellation path above,
            // which forces host_primitive_type to kTriangleList per-draw.
            cacheable.index_buffer_type =
        ...
        case xenos::PrimitiveType::kTriangleStrip:
          ...
          if (builtin_ib_offset_triangle_strips_to_lists_ != SIZE_MAX) {
            ...
            // No pre-baked IB: force_convert_triangle_strips_to_lists cvar is
            // off (the default on all backends). Reached via the tessellation
            // path above, which forces host_primitive_type to kTriangleList
            // per-draw.
            cacheable.index_buffer_type =
                ProcessedIndexBufferType::kHostConverted;
            auto host_indices = reinterpret_cast<uint16_t*>(
