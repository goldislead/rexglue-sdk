...
bool D3D12ZPDQueryPool::EnsureInitialized(const ui::d3d12::D3D12Provider& provider,
  ...
  if (!rtv_initialized) {
    ...
    if (FAILED(device->CreateQueryHeap(&heap_desc, IID_PPV_ARGS(&query_heap_)))) {
      REXGPU_WARN(
          "D3D12ZPDQueryPool: failed to create the ZPD query heap; "
    ...
    if (FAILED(device->CreateCommittedResource(&ui::d3d12::util::kHeapPropertiesReadback,
      ...
      REXGPU_WARN(
          "D3D12ZPDQueryPool: failed to allocate the ZPD query readback "
          "buffer; falling back to fake sample counts");
      Shutdown();
      return false;
    }

    D3D12_RANGE read_range = {0, sizeof(uint64_t) * requested_capacity};
    void* mapping = nullptr;
    if (FAILED(readback_buffer_->Map(0, &read_range, &mapping))) {
      REXGPU_WARN(
          "D3D12ZPDQueryPool: failed to map the ZPD query readback buffer; "
          "falling back to fake sample counts");
      Shutdown();
      return false;
    }
  ...
  ui::d3d12::util::FillBufferResourceDesc(counter_buffer_desc,
  ...
  if (FAILED(device->CreateCommittedResource(
          ...
          &counter_buffer_desc, D3D12_RESOURCE_STATE_UNORDERED_ACCESS, nullptr,
          IID_PPV_ARGS(&rov_counter_buffer_)))) {
    REXGPU_WARN("D3D12ZPDQueryPool: failed to allocate the ZPD ROV counter buffer");
    return false;
  }

  D3D12_RESOURCE_DESC readback_buffer_desc;
  ui::d3d12::util::FillBufferResourceDesc(
      readback_buffer_desc, sizeof(uint32_t) * requested_capacity, D3D12_RESOURCE_FLAG_NONE);
  if (FAILED(device->CreateCommittedResource(
          &ui::d3d12::util::kHeapPropertiesReadback, provider.GetHeapFlagCreateNotZeroed(),
          &readback_buffer_desc, D3D12_RESOURCE_STATE_COPY_DEST, nullptr,
  ...
  D3D12_RANGE read_range = {0, sizeof(uint32_t) * requested_capacity};
  void* mapping = nullptr;
  if (FAILED(rov_counter_readback_buffer_->Map(0, &read_range, &mapping))) {
    REXGPU_WARN("D3D12ZPDQueryPool: failed to map the ZPD ROV counter readback buffer");
    rov_counter_readback_buffer_.Reset();
    rov_counter_buffer_.Reset();
    return false;
  }
...
void D3D12ZPDQueryPool::ReleaseQueryIndex(uint32_t query_index, uint32_t query_generation) {
  ...
  if (!GenerationMatches(query_index, query_generation)) {
    REXGPU_WARN("D3D12ZPDQueryPool: stale release index={} gen={}", query_index, query_generation);
    return;
  }
...
void D3D12ZPDQueryPool::QueueQueryResolve(uint32_t query_index, bool uses_rov_counter) {
  if (query_index >= capacity_) {
    return;
  }
...
void D3D12ZPDQueryPool::ClearROVCounter(DeferredCommandList& deferred_command_list,
                                        uint32_t query_index) const {
...
void D3D12ZPDQueryPool::FlushResolveBatch(DeferredCommandList& deferred_command_list,
  ...
  if (!submission_open ||
      (resolve_batch_indices_.empty() && rov_counter_resolve_batch_indices_.empty())) {
    return;
  }

  auto build_ranges = [this](std::vector<uint32_t>& indices, std::vector<uint8_t>& pending) {
  ...
  build_ranges(rov_counter_resolve_batch_indices_, rov_counter_resolve_batch_pending_);
  for (const ResolveRange& range : resolve_batch_ranges_) {
    ...
    deferred_command_list.D3DCopyBufferRegion(rov_counter_readback_buffer_.Get(), offset,
                                              rov_counter_buffer_.Get(), offset, size);
  }
